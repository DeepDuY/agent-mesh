...
- 随 `poll_for_task` 上报，`heartbeat()` 以注册表 `METRIC_FIELDS` 持久化到 `agents` 表（每次心跳以最新值覆盖；**某字段缺失时保留旧值**，见 [standards/edge-reporting.md](./standards/edge-reporting.md)）；`AgentStatus` 暴露并在 Web 节点详情「系统资源」区展示（探针版本即 `version` 字段）。
...
- **上报**：边沿解析 opencode `--format json` 的 JSONL 事件流（`type=text`→`text`、`type=error`→`error`、`type=complete`→`complete`；`message`/`session`/`step`/`reasoning` 归为 `text`，其余→`raw`），经 `asyncio.Queue` 批量（~0.5s）POST `POST /api/edge/task_log`（`require_any_token`）；command 模式将 stdout 行按 `raw` 上报（**当前未单独上报 stderr**）。上报失败仅 warning，不影响最终结果。
