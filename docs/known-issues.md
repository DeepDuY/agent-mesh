...
27. **2026-09-14 安全：模板/权限接口提权漏洞修复**：
    ...
    - **节点描述模板化**：模板新增独立字段 `templates.node_description`（迁移 `016_template_node_description.sql`；PG 镜像），与模板自身的 `description`（说明/备注）**分开**。生效 `effective_description = 节点自身 description > 模板 node_description`（新增 `TaskStore.describe_agents()`，REST 与 MCP 的节点接口均已填充）。
