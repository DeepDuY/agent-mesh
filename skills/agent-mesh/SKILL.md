...
> ⚠️ **主 Agent 不得创建、修改、删除模板，也不得给节点改绑模板或修改权限。** 这些操作会改变节点的执行权限，属于**提权**行为。为杜绝此类提权，**模板读/写接口与节点改绑模板接口已从 API 移除**：任何 API 调用（curl/MCP，即使持有管理员 token）都会得到 `404`，只能在 Web 管理平台由人通过页面操作。**不要尝试绕过。**
